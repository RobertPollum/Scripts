"""
Patch Bazarr config.yaml directly via SFTP + pct push.
Writes a Python patcher script into the container and runs it.
"""
...
# Write the patcher script to Proxmox /tmp via SFTP
patcher = f"""\
import yaml
...
sftp = c.open_sftp()
with sftp.open("/tmp/patch_bazarr.py", "w") as f:
    f.write(patcher)
sftp.close()

# Push the script into the container
_, o, e = c.exec_command("pct push 112 /tmp/patch_bazarr.py /tmp/patch_bazarr.py --perms 644")
print("push:", o.read().decode().strip(), e.read().decode().strip())

# Stop Bazarr, run patcher, restart
print("Stopping Bazarr...")
pct("112", "systemctl stop bazarr 2>/dev/null || true")
time.sleep(3)

out, err = pct("112", "python3 /tmp/patch_bazarr.py")
print(f"Patcher: {out!r} {err!r}")

# Verify yaml directly
out2, _ = pct("112", f"python3 -c \"import yaml; c=yaml.safe_load(open('{CONF_PATH}')); "
              f"print('sonarr ip:', c['sonarr']['ip'], 'key:', c['sonarr']['apikey'][:8]); "
              f"print('radarr ip:', c['radarr']['ip'], 'key:', c['radarr']['apikey'][:8])\"")
print(f"Yaml content: {out2}")
...
if sonarr.get("apikey") and radarr.get("apikey"):
    ...
    print("\n⚠️  API still shows defaults — Bazarr may overwrite config on startup")
    print(f"   Will need to configure manually at http://{os.environ['BAZARR_IP']}:6767")
