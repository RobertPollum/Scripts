"""
Patch Bazarr config.yaml using PyYAML bundled in /opt/bazarr/libs.
"""
...
patcher = f"""\
import sys
sys.path.insert(0, '/opt/bazarr/libs')
import yaml
...
sftp = c.open_sftp()
with sftp.open("/tmp/patch_b5.py", "w") as f:
    f.write(patcher)
sftp.close()
run_host("pct push 112 /tmp/patch_b5.py /tmp/patch_b5.py --perms 644")

print("Stopping Bazarr...")
pct("112", "systemctl stop bazarr 2>/dev/null || true")
time.sleep(3)

out, err = pct("112", "python3 /tmp/patch_b5.py")
print(f"Patcher: {out}")
if err:
...
if sonarr.get("apikey") and radarr.get("apikey"):
    ...
    print("\n⚠️  Bazarr resets config on startup")
    print(f"   Configure manually at http://{os.environ['BAZARR_IP']}:6767 → Settings → Sonarr / Radarr")
